...
@WebServlet(name = "TCodeListCtl", urlPatterns = { "/ctl/TCodeListCtl" })

public class TCodeListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<TCodeBean> list = model.search(bean, pageNo, pageSize);

			List<TCodeBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					TCodeBean deleteBean = new TCodeBean();
	...
	protected String getView() {
		...
		return ORSView.TCODE_LIST_VIEW;
	}
}